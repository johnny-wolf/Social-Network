	    	...
	    	<div class="row">
	             ...
	             <!-- status window -->
	          	<div class="col-sm-5 col-md-6">
				    ...
				    <c:forEach var="post" items="${posts}" begin="${filterPosts * 10}" end="${(filterPosts * 10) + 10}" varStatus="loop">
				    	<div class = "facebook-comment-box">
			            	<div>
			                	<img src="ref-material-prototype/img/${post.poster.picture}" onerror="this.src='ref-material-prototype/img/default.png'" style="width:75px;height:75px;">
			                ...
			                <div>
			                	<p> ${post.formattedDate} | <a href"">Like</a> ${post.popularity} Comments</p>
		                ...
		                <c:if test="${requestScope.commentActive eq post.id}">
							<p><jsp:include page="/comment.jsp?postId=${post.id}&comments=${requestScope.comments}"/></p>
						</c:if>
				    </c:forEach>
				    <c:forEach begin="0" end="${postCount / 10}" varStatus="loop">
    					<a href="/newsFeed?ownerId=${wallOwnerId}&filterPosts=${loop.index}">${loop.index}</a>
					</c:forEach>
				</div>
